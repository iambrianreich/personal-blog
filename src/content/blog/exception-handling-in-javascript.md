...
description: 'A beginner-friendly guide to understanding and implementing exception handling in JavaScript with try-catch statements.'
pubDate: 'Nov 22 2022'
categories: ['Frontend']
tags: ['exception-handling', 'exceptions', 'javascript', 'programming']
---
...
Most programming languages that support exception handling do so with a very similar structure: The [*try*...*catch*](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/try...catch) statements, and JavaScript is no different.
try {
// Do some work
} catch (error) {
// Handle errors
}
You'll notice there are two code blocks: a *try* block and a *catch* block.  One cannot exist without the other: a *try { ... } *must always be followed by a *catch { ... }.* (**Note: **th*ough this is the most common pattern, technically try can also be followed by finally, but we'll get to that.)*
### The Try Block
...
### The Catch Block
A [catch](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/try...catch) block handles Exceptions thrown from the `try `block that precedes it. It begins with the `catch `keyword, optionally followed by `(error)`, where *error *defines the name of a variable that will reference the Exception, followed by a block `{ ... } `containing the exception handling code.
